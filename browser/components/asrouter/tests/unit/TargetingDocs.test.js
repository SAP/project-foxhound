...
// The following targeting parameters are either deprecated or should not be
// included in the docs for some reason.
const SKIP_DOCS = [];
...
describe("ASRTargeting docs", () => {
  ...
  describe("All targeting params documented in targeting-attributes.md", () => {
    for (const targetingParam of ASRTargetingAttributes) {
      // If this test is failing, you probably forgot to update
      // asrouter/docs/targeting-attributes.md for a new targeting attribute.
  ...
  describe("No extra attributes in targeting-attributes.md", () => {
    ...
    for (const targetingParam of DOCS_TARGETING_HEADINGS.filter(
      ...
      // If this test is failing, you might have spelled something wrong or
      // removed a targeting param without removing its docs.
