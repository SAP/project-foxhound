...
class TestHarness:
    ...
    def equals(self, a, b, msg):
        if json.dumps(a, sort_keys=True) == json.dumps(b, sort_keys=True):
            self.test_pass(msg)
        else:
            self.test_fail(f"{msg} | Got {a} expected {b}")

    def should_throw(self, description, exception, code):
        threw = "(no exception)"
        try:
            code()
        ...
        if threw.startswith(exception):
            ...
            self.test_fail(
                f"{description} - should throw `{exception}` | Got `{threw}`"
            )


def run_tests(tests, verbose):
    testdir = os.path.join(os.path.dirname(__file__), "tests/build")
    ...
    for test in tests:
        ...
        _test = __import__(testpath, globals(), locals(), ["WebCompatBuildTest"])

        harness = TestHarness(test, verbose)
        harness.start()
        try:
            _test.WebCompatBuildTest.__call__(harness)
    ...
    if verbose or failed_tests:
        ...
        print(f"Unexpected: {sum(len(failures) for _, failures in failed_tests)}")
