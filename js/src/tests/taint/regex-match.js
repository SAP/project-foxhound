...
//same regex
assertDeepEq(localstr, localregex);
...
for(var i in globalregex) {
	...
	assertEq(ntaint.operators[1].op, "match");
	assertEq(ntaint.operators[1].param1, "(\\w+)"); //the pattern
	assertEq(ntaint.operators[2].op, "substring");
}
