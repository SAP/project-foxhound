...
function testRoundtrip(year, month, monthCode, day) {
  const dateFromYearMonth = Temporal.PlainDateTime.from({ year, month, day, hour: 12, minute: 34, second: 56, millisecond: 987, microsecond: 654, nanosecond: 321 }, options);
  TemporalHelpers.assertPlainDateTime(
    dateFromYearMonth,
    year, month, monthCode, day, 12, 34, 56, 987, 654, 321,
    `${dateFromYearMonth} - created from year and month`);

  const dateFromYearMonthCode = Temporal.PlainDateTime.from({ year, monthCode, day, hour: 12, minute: 34, second: 56, millisecond: 987, microsecond: 654, nanosecond: 321 }, options);
  TemporalHelpers.assertPlainDateTime(
