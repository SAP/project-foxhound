...
TemporalHelpers.assertPlainYearMonth(
  date256302.add(years1, options),
  2564, 2, "M02", "add 1y to Feb",
  "be", 2564);

TemporalHelpers.assertPlainYearMonth(
  date256302.add(years4, options),
  2567, 2, "M02", "add 4y to Feb",
  "be", 2567);

TemporalHelpers.assertPlainYearMonth(
  date256302.add(years1n, options),
  2562, 2, "M02", "subtract 1y from Feb",
  "be", 2562);

TemporalHelpers.assertPlainYearMonth(
  date256302.add(years4n, options),
...
TemporalHelpers.assertPlainYearMonth(
  date256301.add(months1, options),
  2563, 2, "M02", "add 1mo to Jan in leap year",
  "be", 2563);

TemporalHelpers.assertPlainYearMonth(
  Temporal.PlainYearMonth.from({ year: 2564, monthCode: "M09", calendar }, options).add(months5),
  2565, 2, "M02", "add 5mo with result in the next year",
  "be", 2565);
TemporalHelpers.assertPlainYearMonth(
  Temporal.PlainYearMonth.from({ year: 2562, monthCode: "M09", calendar }, options).add(months5),
  2563, 2, "M02", "add 5mo with result in the next leap year",
  "be", 2563);

TemporalHelpers.assertPlainYearMonth(
  Temporal.PlainYearMonth.from({ year: 2564, monthCode: "M12", calendar }, options).add(years1months2),
  2566, 2, "M02", "add 1y 2mo with result in the next year",
  "be", 2566);
TemporalHelpers.assertPlainYearMonth(
  Temporal.PlainYearMonth.from({ year: 2565, monthCode: "M12", calendar }, options).add(years1months2),
  2567, 2, "M02", "add 1y 2mo with result in the next leap year",
  "be", 2567);

TemporalHelpers.assertPlainYearMonth(
  date256303.add(months1n, options),
  2563, 2, "M02", "subtract 1mo from Mar in leap year",
  "be", 2563);

TemporalHelpers.assertPlainYearMonth(
  date256301.add(months11n),
  2562, 2, "M02", "subtract 11mo with result in the previous year",
  "be", 2562);
TemporalHelpers.assertPlainYearMonth(
  Temporal.PlainYearMonth.from({ year: 2564, monthCode: "M01", calendar }, options).add(months11n),
  2563, 2, "M02", "add 11mo with result in the previous leap year",
  "be", 2563);

TemporalHelpers.assertPlainYearMonth(
  Temporal.PlainYearMonth.from({ year: 2565, monthCode: "M04", calendar }, options).add(years1months2n),
  2564, 2, "M02", "add 1y 2mo with result in the previous year",
  "be", 2564);
TemporalHelpers.assertPlainYearMonth(
  Temporal.PlainYearMonth.from({ year: 2564, monthCode: "M04", calendar }, options).add(years1months2n),
