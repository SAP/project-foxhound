...
AbortReasonOr<Ok> IonBuilder::getElemTryString(bool* emitted, MDefinition* obj,
                                               MDefinition* index) {
  MOZ_ASSERT(*emitted == false);
  // TaintFox: this function is currently disabled as it prevents taint propagation for
  // string element access. This is due to the fact that this function effectively turns
  // `str[i]` into `String.fromCharCode(str.charCodeAt(2))`, which we cannot propagate taint
  // through.
  return Ok();
