...
function test() {
  ...
  BrowserTestUtils.browserLoaded(newBrowser, false, null, true).then(
    async () => {
      await SpecialPowers.spawn(newBrowser, [], function () {
        is(
  ...
  SpecialPowers.spawn(newBrowser, [chromeURL], function (url) {
