...
For JavaScript properties, if there are small number of matching properties in the WebIDL definitions, "Possible IDL definitions" submenu is shown.
This menu item is shown only when there are 3 or less numbers of properties with the same name across WebIDL interfaces.

.. image:: img/context-menu-possible.png
    :class: border
    :alt: The context menu for possible symbols
    :width: 648px

Possible IDL definitions
  Opens a sub menu for IDL symbols with the property name, which can be the property the code is using.
