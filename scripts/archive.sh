...
node "${SCRIPT_FOLDER}/install-preferences.js" "${OBJ_FOLDER}/dist/foxhound"

if ! [[ -d "$OBJ_FOLDER/dist/foxhound" ]]; then
...
if [[ "$(uname)" == "Linux" ]]; then
  cp /usr/lib/x86_64-linux-gnu/libstdc++.so.6 "${OBJ_FOLDER}/dist/foxhound/libstdc++.so.6"
elif [[ "$(uname)" == MINGW* || "$(uname)" == MSYS* ]]; then
  # Bundle vcruntime14_1.dll - see https://github.com/microsoft/playwright/issues/9974
  cd "$(printMSVCRedistDir)"
  cp -t "${OBJ_FOLDER}/dist/foxhound" vcruntime140_1.dll
fi

# tar resulting directory and cleanup TMP.
cd "${OBJ_FOLDER}/dist"
zip -r "$ZIP_PATH" foxhound
