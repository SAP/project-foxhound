...
.. _uploading-symbols-for-a-try-build:

Uploading symbols for a try build
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

We provide an alternate URL ``https://symbols.mozilla.org/try`` which can be
used in place of the official URL. The only difference is that when you use the
alternate URL, you'll also be able to retrieve symbols from try builds. Note
that you do not need both URLs; despite its name, the `try` endpoint-family
will return symbols from both release and try builds (in that order).

The upload occurs through ``upload-symbols`` jobs. For example, the
``build-win64-shippable/opt-upload-symbols`` job will upload the symbols
generated by the ``build-win64-shippable/opt`` job. These jobs are available on
**try** and **release branches** (mozilla-central, mozilla-beta,
mozilla-release, ESR branches). They are **not** available on **autoland**: the
full crash symbol archive is not produced and the upload task is not generated
for autoland pushes.

On release branches (e.g. mozilla-central), ``upload-symbols`` jobs run
automatically for all builds that have full crash symbols enabled. On try, the
jobs exist in the task graph but only run if explicitly selected (via ``./mach
try fuzzy`` or by manually triggering them in Treeherder).

To upload the symbols for an already completed build job in an existing try
push, you can use **Add new jobs** in the treeherder interface. Next to the
**B** job for the build, you will find the symbol job listed as **Sym**
(assuming that we have this job for the kind of build you are looking at).
Click to select it, then at the top of the page you will now be able to hit
**Trigger 1 New Job** to start the upload task. Once the task completes as
green in treeherder, your build will now be listed by the Mozilla Symbol Server
when using the alternate URL.
