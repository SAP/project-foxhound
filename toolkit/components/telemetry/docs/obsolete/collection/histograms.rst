...
Histograms should be declared in the :searchfox:`Histograms.json <toolkit/components/telemetry/Histograms.json>` file. These declarations are checked for correctness at :searchfox:`compile time <toolkit/components/telemetry/gen_histogram_data.py>` and used to generate C++ code.
...
A Telemetry probe is the code that measures and stores values in a histogram. Probes in privileged JavaScript code can make use of the :searchfox:`nsITelemetry <toolkit/components/telemetry/nsITelemetry.idl>` interface to get references to histogram objects. A new value is recorded in the histogram by calling ``add`` on the histogram object:
...
Probes in native code can also use the :searchfox:`nsITelemetry <toolkit/components/telemetry/nsITelemetry.idl>` interface, but the helper functions declared in :searchfox:`Telemetry.h <toolkit/components/telemetry/Telemetry.h>` are more convenient:
