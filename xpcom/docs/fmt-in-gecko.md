...
fmt::format_to_n(res, 1024,
                 "Formatting a number: {} and another one: {} "
...
auto format_as(POD aInstance) -> std::string {
  return fmt::format("POD: mA: {}, mB: {}", aInstance.mA, aInstance.mB);
}
```
...
fmt::println("### debug: {}", p);
fmt::print(stderr, "### debug to stderr {}\n", p);
...
auto [out, size] = fmt::format_to(bufFmt, "{:#04x}", fmt::join(array, ", "));
*out = 0; // Write the null terminator
...
aLovelyString.AppendFmt("{}", 4);

nsString aLovelyWideString(u"Here are two values: ");
aLovelyString.AppendFmt(u"{}, {}", 4, u"wide");
```

Or directly use `nsFmt[C]String`:

```c++
nsFmtCString str("{},{},{},{}", 1, 1, 2, 3);
nsFmtString str(u"{},{},{},{}", 1, 1, 2, u"wide string");
// use it as usual
```
