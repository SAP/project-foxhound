...
A quick look at the [pile of defects](https://bugzilla.mozilla.orgbuglist.cgi?query_format=advanced&status_whiteboard=%5Bwin%3Atouch%5D&list_id=16586149&status_whiteboard_type=allwordssubstr)
...
> demonstration in [this article](https://www.windowslatest.com/2020/05/21/microsoft-is-adding-elastic-scrolling-to-chrome-on-windows-10/),
...
> ([For the curious](https://socket3.wordpress.com/2019/07/31/windows-for-pen-computing-1-0/),
> and I'm certain [this book](https://www.amazon.com/Microsoft-Windows-Pen-Computing-Programmers/dp/1556154690)
...
> "[Windows Tablet PC](https://learn.microsoft.com/en-us/windows/win32/tablet/tablet-pc-development-guide)"
> APIs, most of which are provided through the main [InkCollector](https://learn.microsoft.com/en-us/windows/win32/tablet/inkcollector-class)
...
>    these events via COM [SystemGesture](https://learn.microsoft.com/en-us/windows/win32/tablet/inkcollector-systemgesture)
>    events using the [InkSystemGesture](https://learn.microsoft.com/en-us/windows/win32/api/msinkaut/ne-msinkaut-inksystemgesture)
...
>    recognize letters, numbers, punctuation, and other [common shapes](https://learn.microsoft.com/en-us/windows/win32/api/msinkaut/ne-msinkaut-inkapplicationgesture)
...
> [Bug 1016232](https://bugzilla.mozilla.org/show_bug.cgi?id=1016232).
...
> touchscreen events can use [RegisterTouchWindow](https://learn.microsoft.com/en-us/windows/win32/api/winuser/nf-winuser-registertouchwindow)
...
> [These MSDN diagrams](https://learn.microsoft.com/en-us/windows/win32/wintouch/advanced-manipulations-overview)
...
> [it is a requirement for Windows 11](https://pocketnow.com/all-windows-11-pcs-will-be-required-to-have-a-precision-touchpad-and-webcam/)
...
> developers of Blender discovered when [they moved to the Pointer API](https://archive.blender.org/developer/D7660).
...
(Note that this section is **not** about the [pointer-events](https://developer.mozilla.org/en-US/docs/Web/CSS/pointer-events)
...
- [Video 1](https://drive.google.com/file/d/1Z1QRSf2RluNhJwkKCzPb6-14vRtkqK8s/view?usp=sharing)
  showcasing overscroll and bounce not working on Surface Pro with touch, but
  other devices/inputs are working
- [Video 2](https://drive.google.com/file/d/1bOgpVGBeZtwelvPJzYdA6uFRpubGtu4W/view?usp=sharing)
...
this is a recent regression in [Bug 1811303](https://bugzilla.mozilla.org/show_bug.cgi?id=1811303)
...
Point #2 is a clear case of the [XY Problem](https://en.wikipedia.org/wiki/XY_problem),
...
1. Firefox handles the `WM_POINTER[LEAVE|DOWN|UP|UPDATE]` messages if the
   ...
   Microsoft warns ([here](https://learn.microsoft.com/en-us/windows/win32/inputmsg/wm-pointercapturechanged))
...
- The logic for the `any-pointer` and `any-hover` queries are objectively
  ...
  [Bug 1813979](https://bugzilla.mozilla.org/show_bug.cgi?id=1813979))
...
- On a separate-but-related note, the [W3C suggested](https://www.w3.org/TR/mediaqueries-5/#descdef-media-pointer)
...
- We can see from the video below that Microsoft Edge has quite a bit more
  ...
  ([Link to video](https://drive.google.com/file/d/14XVLT6CNn2RaXcHHCRIrQmRwoMYjj6fu/view?usp=sharing))
...
- Does it make sense to combine the touchscreen and pen handling together
  ...
  - Note that there is precedent for this, as [GTK](https://gitlab.gnome.org/GNOME/gtk/-/merge_requests/1563)
    has already done so. It appears that [Blender](https://archive.blender.org/developer/D7660)
  ...
  - Would we also want to roll mouse handling into it using the
    `EnableMouseInPointer <https://learn.microsoft.com/en-us/windows/win32/api/winuser/nf-winuser-enablemouseinpointer>` \_\_
    ...
    all!
  - Nick Rishel sent [this link](http://the-witness.net/news/2012/10/wm_touch-is-totally-bananas/)
...
Android provides the [MotionEvent](https://developer.android.com/reference/android/view/MotionEvent.html)
API that batches historal movements.

Unfortunately, the APIs to do this in Windows are terribly broken. As
[this blog](https://blog.getpaint.net/2019/11/14/paint-net-4-2-6-alpha-build-7258/)
makes clear, [GetMouseMovePointsEx](https://docs.microsoft.com/en-us/windows/win32/api/winuser/nf-winuser-getmousemovepointsex)
...
the [GetPointerInfoHistory](https://docs.microsoft.com/en-us/windows/win32/api/winuser/nf-winuser-getpointerinfohistory)
...
although it [has been asked about in the past](https://lists.w3.org/Archives/Public/public-pointer-events/2014AprJun/0057.html).
