...
:alt: |-
:  Screenshot of raw composition string which is inputting Roman
:  character mode of MS-IME (Japanese)
```

```{image} raw_composition_string.png
:alt: |-
:  Screenshot of raw composition string whose all characters are Hiragana
...
:alt: |-
:  Screenshot of candidate window of MS-IME (Japanese) which converts the
:  selected clause
```
...
[Native IME handlers] section for the detail of each platform's
implementation.

:::{note}
Android widget still does not use `TextEventDispatcher` to dispatch
`WidgetCompositionEvents`, see
[bug 1137567](https://bugzilla.mozilla.org/show_bug.cgi?id=1137567).
:::
...
[Forcibly committing composition] section for the detail.
...
See also [Notifications to IME] section for the detail of sending
notifications.
...
"[Forcibly committing composition]" section for additional information.
...
```{image} ChangJie.png
:alt: |-
:  aScreenshot of ChangJie (Traditional Chinese IME) which puts an
...
[bug 1286157](https://bugzilla.mozilla.org/show_bug.cgi?id=1286157). Then,
