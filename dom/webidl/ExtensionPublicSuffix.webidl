 ...
 *  mach python $SCRIPT_DIR/GenerateWebIDLBindings.py -- publicSuffix
 *
...
// WebIDL definition for the "publicSuffix" WebExtensions API
[Exposed=(ServiceWorker), LegacyNoInterfaceObject]
interface ExtensionPublicSuffix {
  // API methods.

  [Throws, WebExtensionStub="ReturnsBoolean"]
  boolean isKnownSuffix(DOMString hostname);

  [Throws, WebExtensionStub="ReturnsOptionalString"]
  DOMString? getKnownSuffix(DOMString hostname);

  [Throws, WebExtensionStub="ReturnsOptionalString"]
  DOMString? getDomain(DOMString hostname, any options);
  [Throws, WebExtensionStub="ReturnsOptionalString"]
  DOMString? getDomain(DOMString hostname);
};
