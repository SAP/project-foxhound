...
void URLParams::DecodeString(const nsACString& aInput, nsAString& aOutput) {
  ...
  for (const char* iter = aInput.BeginReading(); iter != end;) {
    ...
    if (*iter == '%') {
      ...
      if (first != end && second != end && asciiHexDigit(*first) &&
        ...
        unescaped.Taint().concat(
          aInput.Taint().subtaint(static_cast<size_t>(iter - stringstart), static_cast<size_t>(second - stringstart)),
          unescaped.Length());
    ...
    // Taintfox: TODO fix Distance
    unescaped.Taint().concat(
      aInput.Taint().subtaint(static_cast<size_t>(iter - stringstart), static_cast<size_t>(iter - stringstart)+1),
      unescaped.Length());
