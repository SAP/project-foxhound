...
nsHtml5StreamParser::SetupDecodingAndWriteSniffingBufferAndCurrentSegment(
    Span<const uint8_t> aPrefix, Span<const uint8_t> aFromSegment, const StringTaint& aTaint) {
  NS_ASSERTION(IsParserThread(), "Wrong thread!");
  mUnicodeDecoder = mEncoding->NewDecoderWithBOMRemoval();
  nsresult rv = WriteStreamBytes(aPrefix, aTaint);
    ...
    switch (mBomState) {
      ...
      case SEEN_UTF_16_LE_FIRST_BYTE:
        if (!aEof && aFromSegment[i] == 0xFE) {
          ...
          return WriteStreamBytes(aFromSegment.From(i + 1), aTaint.safeSubTaint(i + 1, aFromSegment.Length()));
        }
        prefix = utf16le;
        prefixLength = 1 - i;
        mBomState = BOM_SNIFFING_OVER;
...
void nsHtml5StreamParser::DoStopRequest() {
  ...
  if (MOZ_UNLIKELY(mLookingForXmlDeclarationForXmlViewSource)) {
    ...
    nsresult rv;
    if (NS_FAILED(rv = SniffStreamBytes(Span<const uint8_t>(), true, EmptyTaint))) {
      MarkAsBroken(rv);
      return;
    }
  }
...
void nsHtml5StreamParser::DoDataAvailableBuffer(
  ...
  if (!metaBoundaryWithinBuffer && !localFileLimitWithinBuffer) {
    ...
    mBufferedBytes.AppendElement(std::move(aBuffer));
    DoDataAvailable(mBufferedBytes.LastElement(), aTaint);
  } else {
    MOZ_RELEASE_ASSERT(
    ...
    DoDataAvailable(mBufferedBytes.LastElement(), aTaint.safeSubTaint(0, untilBoundary));
    ...
    DoDataAvailable(mBufferedBytes.LastElement(), aTaint.safeSubTaint(untilBoundary, aBuffer.Length()));
  }
...
void nsHtml5StreamParser::DoDataAvailable(Span<const uint8_t> aBuffer, const StringTaint& aTaint) {
  ...
  if (HasDecoder()) {
    ...
    rv = SniffStreamBytes(aBuffer, false, aTaint);
  }
  if (NS_FAILED(rv)) {
    MarkAsBroken(rv);
    return;
