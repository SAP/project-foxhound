...
add_task(async function () {
  ...
  info(
    "Get the list of animation fronts, by the time this executes, the first, " +
      "short, animation should have ended."
  );
  let animationFronts = await animations.getAnimationPlayersForNode(node);
  if (animationFronts.length === 3) {
    ...
    animationFronts = await animations.getAnimationPlayersForNode(node);
  }

  is(animationFronts.length, 2, "2 animations remain on the node");

  is(
    animationFronts[0].state.duration,
    100000,
    "The duration of the first animation is correct"
  );
  is(
    animationFronts[0].state.delay,
    2000,
    "The delay of the first animation is correct"
  );
  is(
    animationFronts[0].state.iterationCount,
    null,
    "The iterationCount of the first animation is correct"
  );

  is(
    animationFronts[1].state.duration,
    300000,
    "The duration of the second animation is correct"
  );
  is(
    animationFronts[1].state.delay,
    1000,
    "The delay of the second animation is correct"
  );
  is(
    animationFronts[1].state.iterationCount,
    100,
